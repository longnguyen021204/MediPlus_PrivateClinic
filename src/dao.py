...
def get_medicines_list():
    return Medicine.query.filter(Medicine.amount > 0).order_by(Medicine.medicine_name).all()
...
def get_details_bill():
    return db.session.query(
        ...
        Medicine.price.__class__('medicine_price'),
        Packages.price.__class__('package_price')) \
